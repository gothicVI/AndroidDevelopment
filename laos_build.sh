...
function pick_unmerged_commits {
    ...
    if [ "${rev}" == "14.1" ]; then
        ...
        repopick_topic n-asb-2021-01 || exit 1
        repopick_topic n-asb-2021-02 || exit 1
    fi
    if [ "${rev}" == "15.1" ]; then
        echo
        #2021-02-05
        source "${HOME}/git/AndroidDevelopment/lineageos-gerrit-repopick-topic.sh"
        repopick_topic O_asb_2021-02 || exit 1
    fi
    if [ "${rev}" == "16.0" ]; then
        echo
        #2021-02-05
        source "${HOME}/git/AndroidDevelopment/lineageos-gerrit-repopick-topic.sh"
        repopick_topic P_asb_2021-02 || exit 1
    fi
    if [ "${rev}" == "17.1" ]; then
        echo
        #2021-02-05
        source "${HOME}/git/AndroidDevelopment/lineageos-gerrit-repopick-topic.sh"
        repopick_topic Q_asb_2021-02 || exit 1
        echo
...
#        repopick -f 266411 2>&1 || exit 1
    ...
    if [ "${rev}" == "18.1" ]; then
        ...
        #2021-01-05
        source "${HOME}/git/AndroidDevelopment/lineageos-gerrit-repopick-topic.sh"
        repopick_topic android-11.0.0_r27 || exit 1
        echo
        #soong: java: Specify larger heap size for metalava
